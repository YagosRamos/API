...
# 1. Clone este repositório
git clone https://github.com/YagosRamos/API.git
...
|---|---|---|
| `POST` | `/pedidos` | Cria um novo pedido (Gera código numérico sequencial automaticamente). |
| `GET` | `/pedidos` | Lista pedidos (Oculta o CPF do cliente). Aceita o filtro `?situacao=` na URL. |
| `GET` | `/pedidos/:codigo` | Busca os dados completos de um pedido específico pelo código. |
| `PUT` | `/pedidos/:codigo` | Atualiza a situação de um pedido. |
| `DELETE` | `/pedidos/:codigo` | Exclui um pedido do banco de dados em memória. |

---

##  Exemplos de Requisições (JSON)

### 1. Inclusão de um novo pedido (POST `/pedidos`)
**Corpo da Requisição:**
```json
{
  "clienteCpf": "123456789",
